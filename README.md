# Disclaimer
If you are not here for asignment 4: dynamic programming and greedy algorithms, a newer version of this reposity most likely exists under the 'master' branch.
...
### Search Multigrid
The following animations are generated by the aStar path finding algorithm. More specifically, the following three animation are the same tiling generated by the same method (with exception that the initial condition on the shift vector is different meaning some differently placed tiles), but the three animations are examples of what happens when we manipulate the source and target bias. Because the distance of any tile from the source is given by a calculated irregular index distance (in number of tiles) from the source, and the target distance is the euclidean distance fromthe current tile to the target, the distance to the target can only be taken as a heuristic, meaning that we have to experiment with scoring function. Thus the score which we use to determine the next tile to evaluate is calculated by multiplying the distance to the source by a constant and the distance to the target by another constant. The First image is an example of a target heavy function, while the latter become more and more source heavy. Note that an entirely source-dependent score function would be equivalent to Dijkstra's algorith.

![aStarAnimation1-5.gif](Examples/aStarAnimation1-5.gif "aStar 1-5")
![aStarAnimation1-10.gif](Examples/aStarAnimation1-10.gif "aStar 1-10")
![aStarAnimation1-15.gif](Examples/aStarAnimation1-15.gif "aStar 1-15")

Here is an example of Dijkstra's algorithm.

![dij.gif](Examples/dij.gif "dij")
